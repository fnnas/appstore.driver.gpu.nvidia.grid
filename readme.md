...
- 已构建内核包：`6.18.18-trim`、`6.18.18.c788-trim`、`6.18.18.c877-trim`、`6.18.18.c938-trim`、`6.18.18.c952-trim`、`6.18.18.c1032-trim`、`6.18.18.c1078-trim`
...
内核驱动包选择规则：先确认 `uname -r`。旧 `6.18.18-trim` 因为同一内核名下存在多组 build，需要再按 build number 选择 `427`、`570`、`587` 或 `717` 包。其他带构建号的完整内核名直接按 `uname -r` 选择，例如 `6.18.18.c1078-trim` 选择 `6.18.18.c1078-trim-amd64` 包。没有对应预构建包时，可改用 `chroot-amd64` 源码包。
...
| `6.18.18.c1078-trim` | `#1078`      | `nvidia.ko-580.159.03-3-6.18.18.c1078-trim-amd64.tgz.fpk` |
...
nvidia.ko-535.309.01-1-6.18.18.c1078-trim-amd64.tgz.fpk
