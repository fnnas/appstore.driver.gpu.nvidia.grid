...
内核驱动包选择规则：先确认 `uname -r`。旧 `6.18.18-trim` 因为同一内核名下存在多组 build，需要再按 build number 选择 `427`、`570`、`587` 或 `717` 包。其他带构建号的完整内核名直接按 `uname -r` 选择，例如 `6.18.18.c938-trim` 选择 `6.18.18.c938-trim-amd64` 包；如果没有对应包，安装时会提示模块目录不存在。
