...
- 内核版本：`6.18.18-trim`、`6.18.18.c788-trim`、`6.18.18.c877-trim`、`6.18.18.c938-trim` 或 `6.18.18.c952-trim`
- build number：`#473`。只有旧 `6.18.18-trim` 需要用它选择分组包。

旧 `6.18.18-trim` 因为同一内核名下有多组 build，需要按 build number 选择 `427`、`570`、`587` 或 `717` 包。`6.18.18.c788-trim`、`6.18.18.c877-trim`、`6.18.18.c938-trim`、`6.18.18.c952-trim` 这类带构建号的完整内核名直接按 `uname -r` 选择对应包，例如 `6.18.18.c952-trim-amd64`。后续如果系统显示其他完整内核名，应优先下载与 `uname -r` 对应的预构建包；没有对应预构建包时可以使用 `chroot-amd64` 源码包。

本项目目前提供以下内核驱动包：

| 当前内核 / build | 应选择的内核驱动包 |
| --- | --- |
| `#427` 到 `#569` | `nvidia.ko-580.159.03-3-6.18.18-trim-427-amd64.tgz.fpk` |
| `#570` 到 `#586` | `nvidia.ko-580.159.03-3-6.18.18-trim-570-amd64.tgz.fpk` |
| `#587` 到 `#716` | `nvidia.ko-580.159.03-3-6.18.18-trim-587-amd64.tgz.fpk` |
| `#717` 到 `#787` | `nvidia.ko-580.159.03-3-6.18.18-trim-717-amd64.tgz.fpk` |
| `#788` 且内核为 `6.18.18.c788-trim` | `nvidia.ko-580.159.03-3-6.18.18.c788-trim-amd64.tgz.fpk` |
| `#877` 且内核为 `6.18.18.c877-trim` | `nvidia.ko-580.159.03-3-6.18.18.c877-trim-amd64.tgz.fpk` |
...
| --- | --- |
| `#427` 到 `#569` | `nvidia.ko-535.309.01-1-6.18.18-trim-427-amd64.tgz.fpk` |
| `#570` 到 `#586` | `nvidia.ko-535.309.01-1-6.18.18-trim-570-amd64.tgz.fpk` |
| `#587` 到 `#716` | `nvidia.ko-535.309.01-1-6.18.18-trim-587-amd64.tgz.fpk` |
| `#717` 到 `#787` | `nvidia.ko-535.309.01-1-6.18.18-trim-717-amd64.tgz.fpk` |
| `#788` 且内核为 `6.18.18.c788-trim` | `nvidia.ko-535.309.01-1-6.18.18.c788-trim-amd64.tgz.fpk` |
| `#877` 且内核为 `6.18.18.c877-trim` | `nvidia.ko-535.309.01-1-6.18.18.c877-trim-amd64.tgz.fpk` |
...
如果系统是 `6.18.18.c938-trim #938`，应选择 `6.18.18.c938-trim-amd64` 包：

```text
nvidia.ko-580.159.03-3-6.18.18.c938-trim-amd64.tgz.fpk
```

如果系统是 `6.18.18.c952-trim #952`，应选择 `6.18.18.c952-trim-amd64` 包：
...
```text
nvidia.ko-580.159.03-3-6.18.18-trim-427-amd64.tgz
nvidia.ko-580.159.03-3-6.18.18-trim-427-amd64.tgz.fpk
```
